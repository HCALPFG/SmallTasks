...
TString fc_thresh = "0";
TString which_bx = "laser"; //Choose "led" or "laser" (in a train)
...
vector<unsigned int> laser = {258027, 258189, 258190, 258362, 258552, 259142, 259424, 259702, 259912, 259984};
vector<unsigned int> led   = {258031, 258182, 258352, 258534, 258539, 258809, 258893, 259026, 259136, 259419, 259737,
			      259917, 259976, 259987};
vector<unsigned int> run;
unsigned int Nrun=0; //Set after setting run
...
void HFTimingLocal(){ 
  ...
  if(which_bx.Contains("laser")){
    ch->Add("/afs/cern.ch/work/r/rbhandar/public/hcaltuples/localruns/laser/*.root");
    run = laser;
    cout<<"\n[HF Timing Local] Configuration: Setting for laser runs"<<endl;
  }
  if(which_bx.Contains("led")){
    ch->Add("/afs/cern.ch/work/r/rbhandar/public/hcaltuples/localruns/led/*.root");
    run = led;
    cout<<"\n[HF Timing Local] Configuration: Setting for led runs"<<endl;
  }
  Nrun = run.size();

  int dir = gSystem->mkdir(outdir,true);
  if(dir==0){
    cout<<"\n[HF Timing Local] Configuration: Creating directory: "<<outdir<<"\n"<<endl;
    gSystem->CopyFile("/afs/cern.ch/user/r/rbhandar/www/index.php",outdir+"index.php");
  }
  else if(dir==-1) 
    cout<<"[HF Timing Local] Configuration: Saving to rbhandar.web.cern.ch/rbhandar/hcal/hftiming/"
	<<which_bx<<"/fc"<<fc_thresh<<"/\n"<<endl;

  //Analysis
  vector< vector<int> > fillScheme = findFillScheme(ch, which_bx, true);

  HFTimingOne(fillScheme, 3, 2, 32, 3, 2);
  if(which_bx=="laser") HFTimingOne(fillScheme, 3, 2, -32, 3, 2); // Laser runs are offset by 25ns for HFm
  else  HFTimingOne(fillScheme, 3, 2, -32, 3, 2);
}
...
void HFTimingOne(vector< vector<int> > goodbxs, int TStoCheck = 2, int TSadjacent = 1, int IETA=999, int IPHI=999, int DEPTH=999){ 
  ...
  for(unsigned int ievent = 0; ievent<nentries; ievent++){
    ...
    if((ievent<100&&ievent%10==0) || (ievent<1000&&ievent%100==0) || (ievent<10000&&ievent%1000==0) || 
      ...
      if (isatty(1)) {
	printf("\r[HF Timing Local] Processing Events: %i / %i (%i%%)",ievent,nentries,static_cast<int>((ievent+1)*100./nentries));
	fflush(stdout);
	...
	if(0){ // DEBUG
	  cout << Form("[HF Timing Local] Subdet=%i IEta=%i IPhi=%i Depth=%i RecEnergy=%.3f\n",
	...
	if(ithisrun==-1) {
	  if(run_!=skiprun){
	    cout<<"\n\e[31m[HF Timing Local]\e[0m WARNING: Run = "<<run_<<" is being skipped!"<<endl;
	    skiprun = run_;
	    continue;
	  }
	  else		    
  ...
  for(unsigned int irun=0; irun<Nrun; irun++){  
    ...
    if(h2profile[irun][0]->GetEntries()<=11) cout<<"\e[31m[HF Timing Local]\e[0m WARNING: Low stats run = "<<run[irun]<<endl;
  ...
  cout<<"[HF Timing Local] Processed channel "<<IETA<<":"<<IPHI<<":"<<DEPTH<<" in "<<seconds<<" seconds ("<<hoursMinSec(seconds)<<") for "<<nentries
...
vector< vector<int> > findFillScheme(TChain *ch, TString bxtype="iso", bool printbx=false){
  ...
  if(bxtype=="laser" && bxtype=="led"){
    cout<<"[HF Timing Local] Skipping fill scheme for local runs"<<endl;  
  }
  ...
  for(unsigned int iloop=0; iloop<1; iloop++){
    ch->GetEntry(iloop);   
    ...
    if((iloop<100&&iloop%10==0) || (iloop<1000&&iloop%100==0) || (iloop<10000&&iloop%1000==0) || 
      ...
      if (isatty(1)) {
	printf("\r[HF Timing Local] Analyzing fill scheme: %i / %i (%i%%)",iloop,nentries,static_cast<int>((iloop)*100./nentries));
	fflush(stdout);
	if(iloop+1==nentries){
	  printf("\r[HF Timing Local] Analyzing fill scheme: %i / %i (%i%%)",iloop+1,nentries,static_cast<int>((iloop+1.)*100./nentries));
	  fflush(stdout);
	  printf("\n");
	} 
      }
  ...
  for(unsigned int ihist=0; ihist<h_bxs.size(); ihist++){
    ...
    if((ihist%1)==0){
      if (isatty(1)) {
	printf("\r[HF Timing Local] Extracting fill scheme: %i / %i (%i%%)",ihist,(int)h_bxs.size(),static_cast<int>((ihist)*100./h_bxs.size()));
	fflush(stdout);
	if(ihist+1>=h_bxs.size()){
	  printf("\r[HF Timing Local] Extracting fill scheme: %i / %i (%i%%)",ihist+1,(int)h_bxs.size(),static_cast<int>((ihist+1)*100./h_bxs.size()));
	  fflush(stdout);
	  printf("\n");
	}
      }
  ...
  if(printbx){
    ...
    file.close();
    cout<<"[HF Timing Local] Writing bxs: fillscheme.txt"<<endl;      
  }
  ...
  cout<<"[HF Timing Local] Found fill scheme in "<<seconds<<" seconds ("<<hoursMinSec(seconds)<<") for "<<addCommas(nentries)
      <<" events -> "<<roundNumber(hertz,1,1000)<<" kHz, "<<roundNumber(1000,2,hertz)<<" ms per event"<<endl;
...
void makeTextFile(TH1F* timing[][4], int ieta){
  ...
  cout<<"[HF Timing Local] Writing values: "<<name<<endl;
}
